  ...
  <xsl:template match="/">
    <html>
	  ...
	  -->
        ...
        <style type="text/css">
			/* Added by Greg */
			body, div {font-family: helvetica; font-size:7pt;}
			h2 {font-size:9pt;font-weight:bold;}
			h3 {font-size:9pt;font-weight:normal;margin-left:12px;}
			h4 {font-size:9pt;font-weight:normal;margin-left:24px;}
...
-->
  ...
  <xsl:template match="PART">
		...
		<div>
			<xsl:apply-templates select="CONTENTS/SUBPART"/>
			<xsl:apply-templates select="CONTENTS/SUBJGRP"/>
			<xsl:apply-templates select="CONTENTS"/>
		</div>
  </xsl:template>

  <xsl:template match="CONTENTS/SUBPART">
	<div><h3><xsl:value-of select="HD" /></h3></div>
	<xsl:for-each select="SUBJGRP">
		<div><h4>Subjgrp&#x2014;<xsl:value-of select="HD" /></h4></div>
  ...
  <xsl:template match="CONTENTS/SUBJGRP">
		<div><h4>Subjgrp&#x2014;<xsl:value-of select="HD" /></h4></div>
			<!--xsl:for-each select="SECTNO">
				<div style="margin:4px 0px 0px 8px;">
					<xsl:value-of select="." />&#x2014;<xsl:value-of select="(following-sibling::*)" /> 
  ...
  <xsl:template match="CONTENTS">
      <div><h4><xsl:value-of select="HD" /></h4></div>
			<!--xsl:for-each select="SECTNO">
				<div style="margin:4px 0px 0px 8px;">
					<xsl:value-of select="." />&#x2014;<xsl:value-of select="(following-sibling::*)" /> 
				</div>
